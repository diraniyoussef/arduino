...
// GND ------- GND   |R| 240 Ohm
// 3.3V ------ VCC ---'
// D1 -------- SCL (need to be pulled up, and it's already pulled up in the CJMCU (he named it AP3216))
// D2 -------- SDA (need to be pulled up, and it's already pulled up in the CJMCU)
...
boolean cmdStartsWith(const char *st) { // checks if cmd starts with st
  for(int i=0; ; i++) {
    ...
    if(cmd[i]==0) return false; //can be omitted since included in the following line
...
void loop() {
  ...
  //getting from the user Serial input
  ...
  if(ps_on) {
    ...
    /*IDK, I'm reading the low byte before the high byte because here http://www.datasheet-pdf.com/PDF/AP3216C-Datasheet-LITE-ON-1016217
     * they said so in page 17, last paragraph.*/
