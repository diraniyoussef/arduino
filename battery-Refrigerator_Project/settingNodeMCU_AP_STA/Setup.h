...
private:
  ...
  void updateFirstAndLastIndices(int *first_index, int *last_index)
  {                                   // there has to be max_search_interval argument to be passed
    *first_index = (*last_index) + 1; /*the index of the first useful char*/
    *last_index = *first_index;
    while (buffer_info[*last_index] != trailor && (*last_index) < Max_AP_Buffer_Size)
    { /*we're sure we'll find a trailor*/
  ...
  boolean checkIP_HeaderThenCopy(int first_index, int last_index, byte *destination_buff)
  {
    ...
    if (header == 1)
    { // this means that it was 16 in the beginning, but after shifting 4 bits, it's now 1
    ...
    { // all of the 4 bytes must be different than 0
      if (all_bytes_are_non0)
      {
  ...
  void writeNetworkConfigVariablesToEEPROM()
  {
    ...
    { /*we start by writing the determinant, then the SSID, etc...*/
...
public:
  ...
  void check_APmode_pin()
  { /*this method is not called in AP mode*/
    if (NodeMCU::getAP_InPinState())
    { /*user is asking to enter AP mode, e.g. he's pressing a special button for that*/
    ...
    else if (counter_to_start_APmode != max_counter_to_start_APmode)
    {                                                        /*the long pressing has been cancelled.*/
  ...
  void launch()
  { // this method is called everytime the NodeMCU starts
    ...
    check_APmode_pin();
    if (!is_APmode_button_pressed)
    { /*This check is necessary here; the user wants to change network configuration right after NodeMCU startup without falling into a restart (i.e. not being able to change network config)*/
       ...
       */
      if (!fromEEPROM_connectToRouterOrNotify())
      { /*This metod returns false in case the info in the network configuration in the EEPROM was not valid in format, otherwise it returns true*/
  ...
  void runAsAP()
  {
    ...
    if (WiFi.softAPConfig(local_IP, gateway, subnet))
    {
      ...
      if (WiFi.softAP("SetPanelNetConfig", "", 1, false, 1))
      { /*no password, channel is 1 (as the default value), SSID is not hidden,
  ...
  void connectToRouterOrNotify()
  { // this is ("only" - I believe) called on success of reading the buffer containing the network configuration.
    ...
    */
    ...
    { // please follow this order: WiFi.config, then WiFi.mode(WIFI_STA) then WiFi.begin. https://github.com/esp8266/Arduino/issues/2371
      if (determinant == 1)
      {
      ...
      if (determinant == 5 || determinant == 6)
      {
        if (wifi_set_macaddr(STATION_IF, MAC_bytes))
        {
          ...
          { // this allocates a static IP.  //comment for debugging in case this NodeMCU will use the DHCP of the local network router
            if (determinant == 5)
            {
              WiFi.begin(SSID_buff);
            }
        ...
        { // mac value wasn't set for some reason
    ...
    if (NodeMCU::isConnectedToWiFi(true))
    { // checks if connected. This actually waits some time before returning false.
