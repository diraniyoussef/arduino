...
{ /*this involves stopping and resetting the needed things to normal in case we returned back to normal operation*/
...
void loop()
{
  if (AP_op.must_APmode_be_activated)
  {
    ...
    { /*After this gets a (valid) message, it then, from inside, connects to the router (given by the user).
  ...
  else
  {
    ...
    if (!AP_op.must_APmode_be_activated)
    { /*if AP_op.must_APmode_be_activated has just been set to true
      ...
      { /*in case the router was shut down then NodeMCU won't connect. In this case, if AP_op.is_APmode_button_pressed was false then the method isConnectedToWiFi will notify in a specific manner. if it was true, then we don't really want to connect, in fact we don't want then to do anything really.
         */
      ...
      else
      {
        ...
        if (main_oper_vars_need_to_be_reset)
        { /*This is true after any first-time successful connection to router. This will ocur at the start of the NodeMCU, or at the exit of the AP mode when it happens([x], or in case the router disconnects (NodeMCU as a station) then NodeMCU reconnects back[x])*/
