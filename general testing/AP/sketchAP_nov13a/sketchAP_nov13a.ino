...
void setup()
{
  ...
  if (WiFi.softAPConfig(local_IP, gateway, subnet)){
    ...
    if(result == true)
    {
      Serial.println("Ready"); //as a server, do never close the socket with the client, and if you had to after that you sent something to the other side then wait a little before closing
                               // so the other side can receive reliably your message, anyway in this case it's good to receive an ack from him then close.
