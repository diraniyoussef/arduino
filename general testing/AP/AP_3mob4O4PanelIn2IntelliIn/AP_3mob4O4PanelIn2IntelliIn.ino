...
char outPinIndexAsChar[4] = {'3', '4', '6', '9'}; //please put them here in the same order as in the app. It's nicer but not necessary.
                                    //'0' means D0. '1' means D1. 'a' means 10. And so on.
char inPinIndexAsChar[2] = {'7', '8'}; //this is useful in the report the module send to the mobile.
  ...
  if (connectedStationsNumber > 0){
    for(int i = 0; i < MAX_SRV_CLIENTS; i++){      
      ...
      if (possibleNewConnectionMade){ //Only entered once the new connection to any client is established thus a new socket is  established, then an update will be sent to mob. 
                                      // The reason behind this is a bug that happens sometimes, when the module changes but not the app. As if I was accounting on the testAndFixConnection to initiate a new socket.
