...
class NodeMCU{
  ...
  static void lowerAllOutPins() {
    for (int i = 0; i < sizeof( outPinIndexAsChar ); i++ ) {
      digitalWrite( getRealPin( General::getIntFromHexSymbol( outPinIndexAsChar[i] ) ), LOW);            
    }        
  }
  ...
  static int determineEEPROMSize(){
    EEPROM.begin(2 * sizeof(outPinIndexAsChar)); //e.g. 10T1F2T5T6F decoded like this: first byte is mobileId interpreted as a "byte" type, then 0T where 0 is char interpreted
  ...
  static void decodeEEPROM(){    //I believe everything in the EEPROM is of type "byte". https://www.arduino.cc/en/Reference/EEPROMWrite
    ...
    for (int i = 0; i < sizeof(outPinIndexAsChar); i++){
      ...
      if ( !General::arrayIncludeElement(outPinIndexAsChar, sizeof(outPinIndexAsChar), pinChar) || !(pinState == 'F' || pinState == 'T') ) {
  ...
  static void encodeEEPROM(char pin, boolean action){
    ...
    for (i = 0; i < sizeof(outPinIndexAsChar); i++) {
    ...
    if (i == sizeof(outPinIndexAsChar)) {//usually you won't reach here unless it's the first time.
      Serial.print("allocating the pin in EEPROM during encoding... At position: ");
      i = General::getIndexOfArrayElement( outPinIndexAsChar, sizeof(outPinIndexAsChar), pin );
      Serial.println(i);
  ...
  static void checkManualUpdate(){
    ...
    if (digitalRead(D5)){ 
      if (digitalRead( getRealPin( inToOutPin(5) ) ) ){
        digitalWrite( getRealPin( inToOutPin(5) ), LOW );
        encodeEEPROM( getRealPin( inToOutPin(5) ), false);
      } else {
        digitalWrite( getRealPin( inToOutPin(5) ), HIGH );
        encodeEEPROM( getRealPin( inToOutPin(5) ), true);
      }
    }
/*    if (digitalRead(D8)){ 
