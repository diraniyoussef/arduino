...
extern  char outPinIndexAsChar[4];
extern char inPinIndexAsChar[2];
...
/*  void sendAckWithoutId(){
  ...
  */
      ...
      if (reading_buff[lastColonOfIncomingMessage + 1] == 'R' && reading_buff[lastColonOfIncomingMessage + 2] == '?') {//a report request for pins status
        ...
        if (readCharNumber - 1 >= lastColonOfIncomingMessage + 4){
          if (reading_buff[lastColonOfIncomingMessage + 1] == 'O'){
            if ( General::arrayIncludeElement(outPinIndexAsChar, sizeof(outPinIndexAsChar), reading_buff[lastColonOfIncomingMessage + 2]) ) { //to make sure the incoming message indicates the pin clearly //not so needed but good
              //unfortunately String(outPinIndexAsChar).length() is 7 instead of 6 because of the added '\0' char when the buff is converted to String.
  ...
  void sendReport(){
    ...
    int i = 1;   
    String usefulOutMessage; //Out stands for outgoing not output.
    ...
    int pinAsInt;
    //Serial.printf("It should be: %d\n", String(outPinIndexAsChar).length() - 2);
    for (i = 0; i < sizeof(outPinIndexAsChar); i++) { //please note that the real length of outPinIndexAsChar is String(outPinIndexAsChar).length() - 1
      pinAsInt = General::getIntFromHexSymbol( outPinIndexAsChar[i] );
      if ( digitalRead( NodeMCU::getRealPin( pinAsInt ) ) ) { //NodeMCU::getRealPin(i) is the same as saying Di as of D1 or D2 or ...
        stateOfPinString = "T";                       
      } else {
        stateOfPinString = "F";
      }
      usefulOutMessage = usefulOutMessage + "O" + String( outPinIndexAsChar[i] ) + stateOfPinString;      
    }
    //Now to include the state of the input pins, I will consider the following: 
    // If the pins are pulled HIGH then it's T, if they are grounded then it's F.
    for (i = 0; i < sizeof(inPinIndexAsChar); i++){
      pinAsInt = General::getIntFromHexSymbol( inPinIndexAsChar[i] );
      if ( digitalRead( NodeMCU::getRealPin( pinAsInt ) ) ){ //NodeMCU::getRealPin(i) is the same as saying Di as of D1 or D2 or ...
        stateOfPinString = "T";                       
      } else {
        stateOfPinString = "F";
      }
      usefulOutMessage = usefulOutMessage + "I" + String( inPinIndexAsChar[i] ) + stateOfPinString;            
    }    
