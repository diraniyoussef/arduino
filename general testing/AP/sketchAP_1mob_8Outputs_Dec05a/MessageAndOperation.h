...
extern  char includedPinIndexAsChar[];
...
class MessageAndOperation {
  ...
  void sendAck(){//in addition to sending an ack, this method assigns values to outMessageHeader_buff and lastColonOfIncomingMessage
    ...
    //sending the message
    aClient->write((const uint8_t*)totalMessage_buff, totalMessageLength);
  ...
  void sendReport(){
    ...
    if (!onConnectionStart){
      usefulOutMessage = String(outMessageHeader_buff);    //it's like server1:mob1:
    } else {
      NodeMCU::yieldAndDelay(50); //I think this delay may be useful...
      Serial.print("this message should appear on connection.");
      usefulOutMessage = moduleIdStr; //in this case it should hold "server1:" without determining any particular mobile. After all, it is broadcasted to all clients.
      Serial.print("This is moduleIdStr:");
      Serial.print(usefulOutMessage);
      Serial.println();
    }
    ...
    //sending the message
    Serial.println("Now sending a report:");
    Serial.print(usefulOutMessage);
    Serial.println();
