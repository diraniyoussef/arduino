...
class wiFiClient{
  ...
  static const int maxLoopCounterToAssignANewClient = 540; //e.g. every 180 loops (each loop is delayed by 1 second) or 540 loops (each loop is delayed by 300 milliseconds) the client if connected will be deleted - no it's on the stack not the heap
