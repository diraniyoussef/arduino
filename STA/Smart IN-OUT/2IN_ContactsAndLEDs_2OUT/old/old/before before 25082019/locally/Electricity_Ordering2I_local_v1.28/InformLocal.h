...
private:    
  ...
  //const byte Max_Inform_Local_Panels_Number = 6; //this is put in Setup.h now !
  /*
  char* panel_IP[ Max_Inform_Local_Panels_Number ] = { "192.168.111.212", "192.168.111.213" , "192.168.111.214" , "192.168.111.215",
                                          "192.168.111.216" , "192.168.111.217" }; /*PLEASE NOTE THAT 
...
//  char panel_IP[ Max_Inform_Local_Panels_Number ][ Max_IP_Size_As_Buff ]; //this didn't work, so I went with the following.
  /*
  char* panel_IP[ Max_Inform_Local_Panels_Number ] = { "192.168.123.212", "192.168.123.213" , "192.168.123.214" , "192.168.123.215" ,
                                      "192.168.123.216" , "192.168.123.217" };
  const char* panel_id[ Max_Inform_Local_Panels_Number ] = { "11:\0", "12:\0", "13:\0", "14:\0", "15:\0", "16:\0" }; //Not dummy
  int counter_to_await_report_receipt[ Max_Inform_Local_Panels_Number ] = { 0 , 0 , 0 , 0 , 0 , 0 }; //-1 means not awaiting and 0 means to send right away
  ...
  const AsynchroClient* asynchro_client_pointer_array[ Max_Inform_Local_Panels_Number ] = { &asynchro_client[0], &asynchro_client[1] , 
                    &asynchro_client[2] , &asynchro_client[3] , &asynchro_client[4] , &asynchro_client[5] };  
  */
...
public:  
  void setup( ) {
    ...
    for( int i = 0 ; i < obeying_IPs.panels_number ; i++ ) {
      //asynchro_client[i].setAsynchroClient( (char*) panel_IP[i] , 3552 , 3553 ); //usually all local clients listen on these two ports as being servers.
      asynchro_client[i].setAsynchroClient( (char*) panel_IP[i] , 11357 , 11358 );
    }
  ...
  void stopOperations() {
    obeying_IPs.other_IPs_gotten = false;
    obeying_IPs.panels_number = 0; //this has been set in getObeyingIPsFromEEPROM method
    for( int i = 0 ; i < obeying_IPs.panels_number ; i++ ) {
