...
private:   
  ...
  static const int local_server_mob_Number = 33;
  const char* local_server_mob_i_Id_buff[ local_server_mob_Number ] = { "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0",
  ...
  }; //e.g.(mob_Id_buff + 2)[4] will the 'M' ???  
...
class LocalServer {
private:
  static const int Max_Concurrent_Clients = 15;
...
//            Serial.printf("client_i = %d of port_i = %d was once connected but not now.\n", client_i, port_i);     
    ...
    for(int port_i = 0; port_i < Local_Server_Number; port_i++) {
      for(int client_i = 0; client_i < Max_Concurrent_Clients; client_i++) {
        ...
        if( *theClient ) {
          if( theClient->connected() ) {
            Serial.printf("LocalServer. client_i = %d of port_i = %d is always connected.\n", client_i, port_i);
            if( theClientObj->m_and_op.getMessage() ) {
              //process received message
              Serial.printf("LocalServer. A message is received from client_i = %d of port_i = %d\n.", client_i, port_i);
