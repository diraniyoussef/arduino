...
public:
  ...
  void processMessage() {    
    if( analyze() ) {
      ...
      NodeMCU::yieldAndDelay();      
      RemoteServerMessageOp::updatePinAndEEPROM( the_request );
      sendReport();
    } else {
...
private:
  ...
  LocalIncomingClient client_[ MAX_SRV_CLIENTS ][ Local_Server_Number ];

  void stopAllClientsOnPort( int port_i ) {
    for( int client_i = 0 ; client_i < MAX_SRV_CLIENTS ; client_i++ ) {
      if( client_[client_i][port_i].theClient ) {
    ...
    for( int port_i = 0 ; port_i < Local_Server_Number ; port_i++ ) {
      for( int client_i = 0 ; client_i < MAX_SRV_CLIENTS ; client_i++ ) {
        ...
        if ( clientToBeCreated ) {
          ...
          theClientObj->counter_to_assign_a_new_client = 0; 
          theClientObj->m_and_op.reflected_id_buff[0] = '\0';
