...
static const int local_server_mob_Number = 3;
const char* local_server_mob_i_Id_buff[ local_server_mob_Number ] = { "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0"}; //e.g.(mob_Id_buff + 2)[4] will the 'M' ???  
...
class LocalServer {
private:
  static const int Max_Srv_Clients = 3;
  LocalIncomingClient client_[ Max_Srv_Clients ][ SERVERS_NUMBER_TO_MAINTAIN_CONNECTION ];
...
public:
  void process() {
    ...
    for( int port_i = 0 ; port_i < SERVERS_NUMBER_TO_MAINTAIN_CONNECTION ; port_i++ ) {
...
//              Serial.printf("client_i = %d of port_i = %d will be stopped to free resources.\n", client_i, port_i);
        ...
        if ( clientToBeCreated ) {
          ...
          theClientObj->m_and_op.mob_id_buff[0] = '\0';
