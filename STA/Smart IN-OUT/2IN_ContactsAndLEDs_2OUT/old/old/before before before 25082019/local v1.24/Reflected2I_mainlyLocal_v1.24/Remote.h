...
private:       
  ...
  static const int mob_Number = 4; //Don't forget to add "dummy" in the intermediate server.
  //const char* mob_i_Id_buff[ mob_Number ] = { "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0"};
  const char* mob_i_Id_buff[ mob_Number ] = { dummy_buff, "S4:\0", "TrekStor_Tab:\0", "Mom_Tab:\0" }; //"dummy" thing: If I were to use dummy at all (because ...), and if I were to make sure it is connected to the intermediate relay server, then I would be using the ack mechanism, and the incoming ack will have "dummy" in its signature, then I will have to have "dummy" in mob_i_Id_buff array.
...
private:
  ...
  //message handling variablessendReportMethod = RemoteServ
  char* read_buff;
  int read_char_number = 0;
        ...
        if( read_buff[ test_index + 1 ] == 'O') {
          ...
          if ( General::arrayIncludeElement( (char*) in_pin , In_Pins_Number , read_buff[ test_index + 2 ] ) 
            ...
            if ( read_buff[ test_index + 3 ] == 'T' || read_buff[ test_index + 3 ] == 'F' ) {
              ...
              if( actionType != digitalRead( NodeMCU::getRealPinFromD( General::getIntFromHexChar( read_buff[ test_index + 2 ] ) ) ) ) {
...
*/
  ...
  const char* panel_id[ panels_number ] = { "5:\0" }; 
...
//    inform_remote_setter.setup( &inform_remote ); //To be commented
    ...
    for( int incoming_message1_index = 0 ; incoming_message1_index < AsynchroClient::max_incoming_messages_number ; incoming_message1_index++ ) {
      if( remote_connection.message_in_processing_from_client_1[ incoming_message1_index ] ) { 
        if( remote_server_message_op.getMessage( (char*) remote_connection.buff_1[ incoming_message1_index ], 
          ...
          remote_server_message_op.processMessage( &is_ack_hi_received_1 );          
...
/*
          inform_remote.checkIncomingMessageAndFixCounter( (char*) remote_connection.buff_1[ incoming_message1_index ], 
                                                            remote_connection.len_buff_1[ incoming_message1_index ] ); //To be commented
*/
        }
...
/*          
          //Now we check if the message has to do with InformEntity. //To be commented
