...
private:       
  ...
  static const int mob_Number = 3; //Don't forget to add "dummy" in the intermediate server.
  //const char* mob_i_Id_buff[ mob_Number ] = { "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0"};
  const char* mob_i_Id_buff[ mob_Number ] = { dummy_buff, "S4:\0", "TrekStor_Tab:\0" }; //"dummy" thing: If I were to use dummy at all (because ...), and if I were to make sure it is connected to the intermediate relay server, then I would be using the ack mechanism, and the incoming ack will have "dummy" in its signature, then I will have to have "dummy" in mob_i_Id_buff array.
