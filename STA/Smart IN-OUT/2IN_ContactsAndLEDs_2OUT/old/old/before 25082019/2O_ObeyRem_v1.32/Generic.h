...
#include <EEPROM.h>
#include "PCF_.h"
//this is super interesting https://arduino-esp8266.readthedocs.io/en/latest/esp8266wifi/station-class.html for all connection matters
...
//    pinMode(D7, OUTPUT);  
  ...
  static boolean getInPinStateAsConsidered( int D_pin ) {//This is in accordance with PCB design
    return ( !digitalRead( NodeMCU::getRealPinFromD( D_pin ) ) ); ////usually this is in coherence with setOutPinStateAsConsidered
  }
  static boolean getInPinStateAsOpposite( int D_pin ) {//This is in accordance with PCB design
    return ( !digitalRead( NodeMCU::getRealPinFromD( D_pin ) ) ); //here we're reading it as it is
  }
...
//    EEPROM.begin( 2 * PCF::absolute_max_pins_number ); //e.g. 10T1F2T5T6F decoded like this: first byte is moduleId interpreted as a "byte" type, then 0T where 0 is char interpreted
  ...
  static void setOutPinStateAsConsidered( int D_pin , boolean state_bool ) {/*I mean by "As Considered" that I may choose to consider 
    ...
    * These are used with the out pin D3 and D4. I.e. when say D3 is LOW then the corresponding LED is lit (ON).
     */
    digitalWrite( NodeMCU::getRealPinFromD( D_pin ), !state_bool ); //usually this is in coherence with getInPinStateAsConsidered
  }
