...
private:  
  ...
  void runAsAP() {
    ...
    if( WiFi.softAPConfig(local_IP, gateway, subnet) ) {
      ...
      if( WiFi.softAP( "SetPanelNetConfig1", "", 1, false, 1 ) ) {/*no password, channel is 1 (as the default value), SSID is not hidden, 
...
public:
  ...
  void check_APmode_pin() {/*only accessed when not in AP mode*/
    if( NodeMCU::getInPinStateAsOpposite( AP_setup_pin ) ) { /*user is asking to enter AP mode, e.g. he's pressing a special button for that*/
  ...
  static IPAddress remote_IPAddress;
...
/*
          IPAddress public_DNS1(8,8,4,4);
          IPAddress public_DNS2(8,8,8,8);
          IPAddress public_DNS;
          if( EEPROM.read(9) == 1 ) {
            public_DNS = public_DNS1;
            EEPROM.write(9, 0);        
            Serial.println("using DNS 8.8.4.4");    
          } else {
            public_DNS = public_DNS2;
            EEPROM.write(9, 1);
            Serial.println("using DNS 8.8.8.8");    
          }
          EEPROM.commit();
*/
          if ( WiFi.config(local_IP, gateway, gateway, subnet) ) {  
          //if ( WiFi.config(local_IP, public_DNS, gateway, subnet) ) {//this allocates a static IP.  //comment for debugging in case this NodeMCU will use the DHCP of the local network router     
          //a bit weird !     WiFi.config(local_ip, gateway, subnet, dns1, dns2)       from https://arduino-esp8266.readthedocs.io/en/latest/esp8266wifi/station-class.html                    
          //if ( WiFi.config(local_IP, gateway, subnet, gateway, public_DNS) ) {  
            if( determinant == 5 ) {
              WiFi.begin( SSID_buff );
            } else if( determinant == 6 ) {
