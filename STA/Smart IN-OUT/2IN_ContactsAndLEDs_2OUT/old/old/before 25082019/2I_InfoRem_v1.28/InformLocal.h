...
private:
  ...
  const char* panel_id[ panels_number ] = { "7:\0" }; //{ "5:\0" , "6:\0" , "7:\0" };
...
public:
  ...
  void setup( ) {
    for( int i = 0 ; i < panels_number ; i++ ) {
      asynchro_client[i].setAsynchroClient( panel_IP[i] , 11357 , 11358 ); //usually all local clients listen on these two ports as being servers.
    }
