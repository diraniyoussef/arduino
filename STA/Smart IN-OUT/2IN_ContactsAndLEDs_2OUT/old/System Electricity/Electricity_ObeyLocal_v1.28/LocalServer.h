...
private:   
  ...
  /*
  static const int Reflected_Panels_Number = 4;   //only 1 panel is the informer (reflected)
  const char* reflected_i_Id_buff[ Reflected_Panels_Number ] = { "4:\0", "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0"  }; //e.g.(reflected_id_buff + 2)[4] will the 'M' ???  Local_Server_Number
  */
  static const int Reflected_Panels_Number = 1;   //only 1 panel is the informer (reflected)
  const char* reflected_i_Id_buff[ Reflected_Panels_Number ] = { "4:\0"  }; //e.g.(reflected_id_buff + 2)[4] will the 'M' ???  Local_Server_Number
  
  /* We should have something like a dedicated array for the mobiles and another for the ordering panels,
...
public:
  ...
  void processMessage() {    
    if( analyze() ) {       //It's ready made to accept either a report request or an order to change pins locally from a panel e.g.
...
class LocalServer {
private:
  static const int Max_Concurrent_Clients = 5;
