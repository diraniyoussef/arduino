...
private:
  ...
  const static byte resetting_pin = 10; /*With 10, then we need a dip switch in the PCB in order to read from the serial input, that is 
      * by isolating the RST pin whenever we want to read Serial.print values.    
      * Whilst with 9, we don't need.
      */
