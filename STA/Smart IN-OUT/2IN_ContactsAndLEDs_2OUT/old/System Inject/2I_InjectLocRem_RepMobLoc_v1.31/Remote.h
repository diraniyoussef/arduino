...
private:       
  ...
  //const char* mob_i_Id_buff[ mob_Number ] = { "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0"};
...
/*  
  static const int mob_Number = 30; //Don't forget to add "dummy" in the intermediate server.
  const char* mob_i_Id_buff[ mob_Number ] = { "S4:\0", "TrekStor_Tab:\0", "S7_Edge:\0", dummy_buff, "user1:\0",
   "usr2:\0", "usr3:\0", "usr4:\0", "usr5:\0", "usr6:\0", "usr7:\0", "usr8:\0", "usr9:\0", "usr10:\0", "usr11:\0", "usr12:\0", "usr13:\0",
    "usr14:\0", "usr15:\0", "usr16:\0", "usr17:\0", "usr18:\0", "usr19:\0", "usr20:\0", "usr21:\0", "usr22:\0", "usr23:\0", "usr24:\0",
    "usr25:\0", "usr26:\0" };
*/
  ...
  void processMessage( boolean* is_ack_hi_received ) { //we have a message when we enter here
    ...
    if( mob_id_buff[0] != '\0' ) {//this is for authentication
      if( analyze( is_ack_hi_received ) ) { //this is to check if message context is intelligible
        ...
        //}
//The following is used if the panel will get remote report requests from mobile app.    //should be commented but it won't hurt
...
class InformEntity {
private:
  boolean* is_awaiting_ack; /*this variable is only used in remote (not local). So we NEED TO TEST THIS 
  each time we want to set or get it if it's null or not. //A side note: it might had been 
  enough to have one is_awaiting_ack variable for all remote entities instead of one for each remote entity, but 
  having one for each remote entity might present a tiny advantage and I will seek for it!*/
  int panels_number;  
  char** panel_id;
  ...
  void sendReportAndSetCounter( int i ) {
    ...
    counter_to_await_report_receipt[ i ] = 0; /*In case no socket was connected (although unlikely), we want to 
      * rush to make a new connection.
      * In case a socket was connected and we sent something in the following statements, then this will be overriden, and it's fine; 
      * we just want to send.
      */
    ...
    /*
     * I've deleted the 2 variables async1_connected and async2_connected. They were local to this method.
    if( !async1_connected && !async2_connected ) { //this is a protection measurement
      counter_to_await_report_receipt[ i ] = 0; //no need for that because when we enter this method, 
                                                // counter_to_await_report_receipt[ i ] is already 0 !
    }
    */
          ...
          */
          if( pin_index != -1 ) {
            ...
            if ( read_buff[ test_index + 3 ] == 'T' || read_buff[ test_index + 3 ] == 'F' ) {
              ...
              updateNodePinAndEEPROM( out_pin[ pin_index ] , actionType ); //this is the out (LED) pin not the IN pin
...
public:    
  ...
  void informAllIfInPinChanged() {
    ...
    //It is weird that we do the following check especially that we check in analyze the coherence between the received report and the IN pins.
    else if( NodeMCU::isInOutIncoherent() ) { /*We're saying here :
      * in case of any incohrence, if we're considering, for any unknown reason, that 'a' relector doesn't need an update, then resend 
      * to all, all over again !
      */
      for( int i = 0 ; i < panels_number ; i++ ) {
  ...
  void checkIncomingMessageAndFixCounter( char* message_buff, int message_length ) {
    ...
    if( entity_index != -1 ) { /*another valid check is if panel_id_buff[0] != '\0'
      * Anyway, this entity_index check means that the sender of this message is one of our informed (or able to be informed) panels.
       */
       /* //I decided to omit the following "if" because I want to listen to every incoming message. I guess it's not harmful.
      if( counter_to_await_report_receipt[ entity_index ] >= 0 ) { /*
        ...
        *  the content of the message is worth of being checked.
         */
        //I decided to allow listening to imcoming messages even if counter_to_await_report_receipt[ entity_index ] was -1
        if( analyze( entity_index ) ) { //if true then we have received a report that reflects our current pins status (coherence check)
          ...
          * As we've received a valid report from it.
          ...
          //think about resending a report... This might help somehow...           
          //sendReportAndSetCounter( entity_index ); 
          /*here I'm not rushing to switch connectin since we're getting something from our connection
            * I'm just rsending instead. Of course sendReportAndSetCounter involves resetting a countdown counter when successfully sending
            * something.
            */ 
