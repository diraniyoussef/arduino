...
byte in_pin[OUT_PINS_NUMBER] = { 0 , 1 , 2 , 8 , 7 }; // for some reason it's 8 then 7
char out_pin_symbol[OUT_PINS_NUMBER] = {'3', '4', '5', '6', '9'}; //'9'

int PCF::absolute_max_pins_number = OUT_PINS_NUMBER;
//************************************************This block is for PCF
uint8_t address_PCF = 0x40; //0x38
...
char out_pin_symbol_PCF1[ out_pins_number_PCF1 ] = {'d', 'd', 'd', 'd'}; //the order between the two arrays out_pin_PCF1 and out_pin_symbol_PCF1 matters.
//************************************************End of block.
...
{  
  ...
  NodeMCU::setPins();    
  NodeMCU::wifiSetup( PCF1 ); //if not connected to WiFi, NodeMCU will restart. PCF1 handed as an argument is just to be deleted it there if decided to restart.
  //EEPROM stuff
