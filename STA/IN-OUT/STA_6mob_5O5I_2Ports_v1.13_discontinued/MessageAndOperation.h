...
//extern  char includedPinIndexAsChar[4];//for debugging
...
class MessageAndOperation {
  ...
  int getLastCharOccurence(char charToCheck, int searchUpToIndex) {
    for (int i = searchUpToIndex - 2 ; i >= 0 ; i--) {//it's searchUpToIndex - 2 and not searchUpToIndex - 1 because
                                              // I don't want to read the null char sent by the mobile.
  ...
  MessageAndOperation(WiFiClient* aClient_arg) { //this constructor is called at the beginning of a new connection. BTW in a new connection we only use the sendReport method
    ...
    //Please note that it is suitable to me that MessageAndOperation instantiate its corresponding client... Because
    // I may change clients (pointers actually) frequently inside a wiFiClient object.
    aClient = aClient_arg;                
  ...
  ~MessageAndOperation() {
    ...
    if (reading_buff)//important because sometimes I'm not sure if it will be instanciated.
      delete reading_buff;
    if (aClient)//this if is not necessary but for extra protection.
      delete aClient;
    if (theRequest)//needed I believe.
      delete theRequest;
    if (outMessageHeader_buff)
  ...
  void sendAck(){//in addition to sending an ack, this method assigns values to outMessageHeader_buff and lastColonOfIncomingMessage    
    ...
    lastColonOfIncomingMessage = getLastCharOccurence(':', readCharNumber);////useful characters in reading_buff are from 0 to readCharNumber - 1
    
    //setting the final array according to size. 
    //Our response will be someting like:         Zaher_home_03884818:panel1:Zaher_home_03884818:mob1:ACK
    int totalMessageLength = moduleIdStr.length() + (lastColonOfIncomingMessage + 1) + usefulResponse.length() + 1 ;
...
RequestType* Request::getRequestType(){  //returning a pointer is fine in C++
