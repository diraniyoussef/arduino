...
extern char out_pin_symbol[6];
//extern char out_pin_symbol[3]; //debugging

extern byte in_pin[1];
...
public:    
  ...
  static boolean arrayIncludeElement(T array[], int arrayLength, T element) {
    //Serial.printf( "arrayLength is %d, and the element to search for is %c\n" , arrayLength, element );
    for (int i = 0; i < arrayLength; i++) {
      //Serial.printf( "array[i] is %c\n" , array[i] );
      if (array[i] == element) {
        //Serial.printf( "element is found\n" );
        return true;
      }
    }
    //Serial.printf( "element not found\n" );
    return false;
  }
/*
  template <typename T>
  ...
  void encodeEEPROM( char out_pin_char, char state ) {
    ...
    for( i = pins_index_min ; i < pins_index_max ; i++ ) {
      if ( EEPROM.read(2*i) == out_pin_char ) {
        ...
        EEPROM.commit();
//        Serial.printf( "Now reading the just written char in EEPROM which is %c\n", (char) EEPROM.read(2*i+1) );
  ...
  boolean addPinsToReport( String* message ) {//it's like decodeEEPROM.
...
class NodeMCU{  
private:
  const static int connectFailureNotifierPin = 8;  
