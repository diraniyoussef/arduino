...
//#include <time.h> //uncomment for debugging
...
char in_pin[OUT_PINS_NUMBER] = { '0', '1', '2', '6'}; 
char out_pin[OUT_PINS_NUMBER] = { '3', '4', '5', '8' }; //'8' is better off as output since I set it to low before restarting and user may press on it while restarting which will physically set it to HIGH thus damaging it

int PCF::absolute_max_pins_number = OUT_PINS_NUMBER;
boolean is_PCF_connected = true;
//************************************************This block is for PCF
uint8_t address_PCF = 0x40; //0x38 - I accidentally put a fake one (not sure how much useful though)
const byte in_pins_number_PCF1 = 4;
byte in_pin_PCF1[ in_pins_number_PCF1 ] = {30, 30, 30, 30}; //the order between the two arrays in_pin_PCF1 and out_pin_PCF1 matters.
const byte out_pins_number_PCF1 = 4;
byte out_pin_PCF1[ out_pins_number_PCF1 ] = {30, 30, 30, 30}; 
char out_pin_symbol_PCF1[ out_pins_number_PCF1 ] = {'d', 'd', 'd', 'd'}; //the order between the two arrays out_pin_PCF1 and out_pin_PCF1 matters.
//************************************************End of block.
...
{  
  ...
  NodeMCU::setPins();    
  WIFI::connectionSetup(); //if not connected to WiFi, NodeMCU will restart. PCF1 handed as an argument is just to be deleted it there if decided to restart.
